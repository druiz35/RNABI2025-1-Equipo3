...
x2 <- seq(-10, 20, length.out = n_length)
X <- expand.grid(x1, x2)
...
lines(sol_rosen2d, type="b",cex=1.5,col="red")
```


```{r}
sol <- as.data.frame(sol_rosen2d)
colnames(sol) <- c("x1", "x2")
sol$step <- 1:nrow(sol)

x1 <- seq(-10, 10, length.out = 100)
x2 <- seq(-10, 20, length.out = 100)
...
animate(p, fps = 10, duration = 5, width = 600, height = 500, renderer = gifski_renderer("rosenbrock_opt_grad_2d.gif"))
```
...
```{r}
write.csv(df_resultados, "rosenbrock_iter_grad.csv", row.names = FALSE)
```


```{r}
tabla_cada_10 <- df_resultados[df_resultados$Iteracion %% 10 == 1 | 
                                 df_resultados$Iteracion == max(df_resultados$Iteracion), ] 

# Mostrar las primeras 10 iteraciones
knitr::kable(tabla_cada_10, caption = "Iteraciones")
...
lines(sol_rosen3d, type="b",cex=1.5,col="red")
...
animate(p, fps = 10, duration = 5, width = 600, height = 500, renderer = gifski_renderer("rosenbrock_opt_grad_3d.gif"))
```
...
lines(sol_ras2d, type="b",cex=1.5,col="red")
```
...
animate(p, fps = 10, duration = 5, width = 600, height = 500, renderer = gifski_renderer("rastrigin_opt_grad_2d.gif"))
```
...
```{r}
write.csv(df_resultados, "rastrigin_iter_grad.csv", row.names = FALSE)
```
